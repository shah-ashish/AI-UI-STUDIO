...
# -------------------------------------------------------------
# 2. Check System Utilities (zstd, curl, python3, git)
# -------------------------------------------------------------
echo "--- [2/7] Checking System Utilities ---"
export DEBIAN_FRONTEND=noninteractive
export NEEDRESTART_MODE=a

if ! command -v zstd >/dev/null 2>&1; then
  echo "Installing zstd archive utility..."
  if command -v apt-get >/dev/null 2>&1; then
    sudo apt-get update -qq || true
    sudo -E apt-get install -y -qq zstd || true
  fi
else
  echo "✓ Required system utilities already present."
fi
